...
class TorchDevice:
    ...
    def mha_llama(self, hidden_states, attention_mask, w_q, w_k, w_v, w_out, n_head, donate, compress_cache, comp_config, input_layernorm, rotary_emb_inv_freq):
        ...
        position_ids = torch.arange(q_len, device=self.dev).unsqueeze(0).repeat(bsz, 1)
    ...
    def mha_gen_llama(self, inputs, attention_mask, w_q, w_k, w_v,
        ...
        position_ids = torch.arange(src_s + tgt_s, device=self.dev).unsqueeze(0).repeat(b, 1)
        cos, sin = rotary_emb(rotary_emb_inv_freq.data, v, position_ids[:, src_s:src_s+tgt_s])
        q, k = apply_rotary_pos_emb(q, k, cos, sin, position_ids)
        ...
        k_new = k.view(b * n_head, tgt_s, head_dim).transpose(0, 1)
        v_new = v.view(b * n_head, tgt_s, head_dim).transpose(0, 1)
